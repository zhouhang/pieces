 ...
 * 用于include其它页面以用于布局,可以用于在freemarker的servlet环境应用中直接include其它http请求
 * </pre>
 * 
 * <br />
 * <b>Freemarker示例使用:</b>
...
public class HttpInclude {
	private final static Logger logger = LoggerFactory.getLogger(HttpInclude.class);
	...
	private void getLocalContent(String includePath, Writer writer)
		...
		if(includePath.startsWith("/")){
			urlString = domain + includePath;
		} else {
			urlString = domain + "/" + includePath;
		}
		getRemoteContent(urlString, writer);
//      暂时不用
...
//			// TODO response.getCharacterEncoding()有可能为null
