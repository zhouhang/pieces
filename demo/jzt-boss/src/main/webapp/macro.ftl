...
<#macro pages page form="" action="" data="" type="" expression="" nullmsg="">
	<div class="page-file fr">
		...
		<#else>
		    ...
		    <span>本页<b>${page.results?size}</b>条记录</span>
...
<#if num??>
	<#if num?c?index_of(".")!=-1 && (num?c?substring(num?c?index_of(".")+1,num?c?length))?length gt 2>
		${num?c?substring(0,num?c?index_of("."))}.${num?c?substring(num?c?index_of(".")+1,num?c?index_of(".")+3)}
	<#else>
		${num?string(format)}
	</#if>
<#else>
