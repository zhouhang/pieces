...
<!-- pageCenter start -->
    <div class="main">
        <div class="page-main">
            <h1 class="title1">会员管理<span class="subhead"> >> 添加会员</span></h1>
            <div class="listbox">
            	<div class="list_basic">
            	<form id="addMemberForm" action="/getMemberManage/addMember" method="post">
                  <dl>
                	<dt>基本资料</dt>
                    <dd>
                        <label><span class="col_red">*</span> 会员名</label>
                        <input class="text-sty2 text-2 col_333" name="userName" ajaxurl="/getMemberManage/memberNameIsHaved" datatype="dn5-25" nullmsg="请设置会员名！" errormsg="会员名5-25个英文字母或数字英文字母的组合！" sucmsg="该会员名未被注册，可以使用"/>
                    </dd>
                    <dd>
                    	<label><span class="col_red">*</span> 登录密码：</label>
                        <input class="text-sty2 text-2 col_333" name="passWord" id="passWord" datatype="*6-16" nullmsg="请设置密码！" errormsg="密码范围在6~16位之间！" type="text">
                        <span class="Validform_checktip"></span>
                    </dd>
                    <dd>
                    	<label></label>
                        <input type="button" class="btn-hui" onclick="javascript:getRomSecret();" value="随机生成密码">
                    </dd>
                    <dd>
                    	<label><span class="col_red">*</span> 公司/姓名：</label>
                        <input class="text-sty2 text-2 col_333" name="realName" datatype="s2-50" nullmsg="真实姓名不能为空！" errormsg="至少4个字符,最多50个字符！" type="text">
                        <span class="Validform_checktip"></span>
                    </dd>
                    <dd>
                    	<label><span class="col_red">*</span> 手机：</label>
                        <input class="text-sty2 text-2 col_333" name="mobile" ajaxurl="/getMemberManage/memberMobIsExist" datatype="mo" nullmsg="请输入手机号码！" errormsg="请输入正确手机号！" type="text">
                        <span class="Validform_checktip"></span>
                    </dd>
                    <dd>
                    	<label> 备注：</label>
                        <textarea name="remark" class="text textarea-1" draggable="false" datatype="*0-500" errormsg="最多500个字符！" ignore="ignore"></textarea>
                        <span class="Validform_checktip"></span>
                    </dd>
                    
                        <div class="mt25 save relative clearfix">   
                            <div align="left" style="margin-left:250px; *margin-left:230px; "><input type="button" id="userSave" class="btn btn-blue fl" value="保 存"/></div>
                        </div>
                    </dl>
                    </form>
               </div>
        ...
        <dd><div class="">
        <form action="/getMemberManage/addAnyUserDealInfo" id="completeInfoForm" method="post">
            <ul class="form">
                <li>
                ...
                <li class="mt25"><div align="left" style="margin-left:165px;" class="relative"><input id="dealBtn" type="button" class="btn btn-blue" value="保 存"/></div> </li>
            </ul>
            </form>
            </div>
            </dd>
        ...
        <dd>
        <form action="/getMemberManage/saveContacterInfo" method="post" id="contacterInfoForm">
        <div class="" id="len">
            <ul id="dynamic_ul_0" class="form">
                <li>
                	<input type="hidden" name="ucUserContact[0].userId" id="userId" class='userId' value=""/>
                ...
                <li>
                    ...
                    <input name="ucUserContact[0].department" class="text-sty2 text-wid210 col_333 mr10" value="" ignore="ignore" datatype="*1-100"  errormsg="不可超过1-100个字符"/>
                    <span class="dis-in-bk">（未做部门分工，可不填写）</span>
                </li>
                <li>
                    <label class="lab">职务：</label>
                    <input name="ucUserContact[0].job" class="text-sty2 text-wid210 col_333 mr10" value="" ignore="ignore" datatype="*1-100"  errormsg="不可超过1-100个字符"/>
                    <span class="dis-in-bk">（未做部门分工，可不填写）</span>
                </li>
                <li>
                    <label class="lab">联系座机：</label>
                    <input name="ucUserContact[0].phone" class="text-sty2 text-wid210 col_333 mr10" value="" ignore="ignore" datatype="n0-12"  errormsg="不可超过12位数字"/>
                </li>
                <li>
                    <label class="lab"><i class="red">*</i> 联系手机：</label>
                    <input id="mobile" name="ucUserContact[0].mobile" class="text-sty2 text-wid210 col_333 mr10" value="" datatype="mo" nullmsg="请输入手机号码" errormsg="需填写有效的11位手机号码"/>
                ...
                <!--<li class="mt20"><div align="center" class="blue f14 text-sty-udline" datatype="removeContact">删 除</div> </li>-->
            </ul>
            <div id="append">
                <div class="mt25 save relative"> 
                    ...
                    <div align="left" style="margin-left:188px; padding-bottom:5px;"><input id="contactSave" type="button" class="btn btn-blue fl" value="保 存"/></div>
                </div>
            </div>
        </div>
        </form>
    		...
    		callback:function(data){
    			if(data.status == 'y'){
    				$('.userId').each(function(){
    					$(this).val(data.userId);
    				})
    				var Alert = '<span class="min-alert"><i></i>保存成功！</span>';
    	            $("#userSave").parent('div').append(Alert);
        		}else if(data.status == 'n'){
        			var Alert = '<span class="min-alert"><i></i>保存失败！</span>';
    	            $("#userSave").parent('div').append(Alert);
        		}else{
        			var Alert = '<span class="min-alert"><i></i>'+data.status+'</span>';
    	            $("#userSave").parent('div').append(Alert);
        		}
    		}
    	})
	...
	$(function(){
		...
		$("#completeInfoForm").Validform({
			...
			callback:function(data){
				if(data.status==1){
					 var Alert = '<span class="min-alert"><i></i>保存成功！</span>';
			         $('#dealBtn').parent('div').append(Alert);
