...
<!-- pageCenter start -->
    <div class="main">
        <div class="page-main">
            <h1 class="title1">订单分润</h1>
            <form action="/depositFinish" method="post" id="queryForm">
                <ul class="form-search ac-search">
                    <li><span>订单编号：</span><input class="text text-4" type="text" name="orderId" value="${query.orderId!''}"/>&nbsp;
                        ...
                        <span>订单完成时间：</span><input class="text text-date" id="date_b" type="text" name="orderDateStart" value="${query.orderDateStart!''}"/> — <input class="text text-date" id="date_e" type="text" name="orderDateEnd" value="${query.orderDateEnd!''}"/>&nbsp;
            ...
            <div class="use-item1 mt20">
                <table class="table-1" width="100%" cellpadding="1" cellspacing="1">
                    <tr>
                        ...
                        <th width="10%" class="bgf5">商品名称</th>
                        <th width="8%" class="bgf5">买家</th>
                        <th width="8%" class="bgf5">卖家</th>
                        <th width="6%" class="bgf5">状态</th>
                        <th width="12%" class="bgf5">订单完成时间</th>
                        <th width="12%" class="bgf5">处理完成时间</th>
                        <th width="10%" class="bgf5">操作人</th>
                    		...
                    		<tr>
		                       ...
		                       <td>
		                        	...
		                        	<#elseif orderDeposit.depositState==4>
		                        		<@shiro.hasPermission name="订单分润-订单分润拒绝理由">
			                        	...
			                        	<@shiro.lacksPermission name="订单分润-订单分润拒绝理由">
		                        ...
		                        <td class="operator">${orderDeposit.operator!''}</td>
		                        <td class="buyer_salesman_name">${orderDeposit.buyerSalesmanName!''}</td>
		                        <td class="seller_salesman_name">${orderDeposit.sellerSalesmanName!''}</td>
		                        <td>
		                        	...
		                        	<@shiro.hasPermission name="订单分润-查看分配详情">
	                        			<a class="col_blue mr10" name="reDetail" href="javascript:;">分配详情</a>
	                        		</@shiro.hasPermission>
			                        <#if orderDeposit.depositState == '1' || orderDeposit.depositState == '4'>
				                        <@shiro.hasPermission name="订单分润-划账"><a href="javascript:;" class="col_blue" name="account" data-orderid="${orderDeposit.orderId!''}" data-buyer="<@tools.money num=orderDeposit.buyerAmount format='0.##'/>" data-seller="<@tools.money num=orderDeposit.sellerAmount format='0.##'/>" data-total="<@tools.money num=orderDeposit.depositAmount format='0.##'/>" data-platform="<@tools.money num=orderDeposit.platformAmount format='0.##'/>" data-account="<@tools.money num=orderDeposit.totalAmount format='0.##'/>">划账</a></@shiro.hasPermission>
			                        </#if>
		                        </td>
		                    </tr>
                    	</#list>
...
<!--划账 弹层-->
<div class="order-popup profit-box" style="width:520px;height:320px;" datatype="profit">
    <div class="close"></div>
        <h2 class="title">分润比例设置：</h2>
    <div id="Set">
        <form id="accountForm" action="/depositFinish/executeDeposit">
            ...
            <ul>
                ...
                <!--update by fanyuna 2015.12.2 划账弹层显示应收款和实际已收款  start-->
                <li><label></label>应收款：<span id="shouldAmount"></span>元&nbsp;&nbsp;实际已收款<span id="totalAmount"></span>元&nbsp;&nbsp;<input type="hidden" name="depositAmount" datatype="total" errormsg="比例之和与总额不一致"/><span class="Validform_checktip"></span></li>
                <li class="col_999"><label></label>若实际收款大于应收款的情况，请核实是否需要将多收款分润个买家</li>
                <!--update by fanyuna 2015.12.2 划账弹层显示应收款和实际已收款  end-->
...
<div class="order-popup re-detail" datatype="reDetail">
   ...
   <table cellspacing="1" cellpadding="1" width="570" bgcolor="#dddddd" class="table-2" align="center">
    ...
    <dl>
        <dd>订单总金额：<label id="all_moeny"></label>元</dd>
        <dd>操作人：<label id="operator"></label></dd>
    ...
    $(function(){
        ...
        $('td a[name=account]').on('click',function(){
        	var _this = $(this),
        		...
        		//add by fanyuna 2015.12.2 增加应收款
        		_shouldAmount = $.trim($(this).data('account'));
        	...
        	$('#accountForm #shouldAmount').text(_shouldAmount);
            bghui();
            $('.order-popup[datatype=profit]').show();
            $('#Set').show();
            $('#See').hide();
        });
        ...
        function getReject(obj){
    		...
    		$.ajax({
	    		type:"POST",
	    		url:"/depositFinish/orderDepositRejectList",
	    		data:{orderId:orderId,remitType:'1'},
	    		dataType:"json",
