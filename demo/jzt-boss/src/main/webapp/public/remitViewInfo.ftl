...
<!-- pageCenter start -->
    <div class="main">
    	<div class="page-main">
           ...
           <#if type==1>
	           <form action="/getRemitManage/doRemit" method="POST" id="remitCheckForm" enctype="multipart/form-data">
	           	<ul class="store-search layout1 mt20">
		              ...
		              <tr bgcolor="#ffffff">
		                    ...
		                    <th>状态：</th>
		                    <td><#if remitStatusEnumMap??><#list remitStatusEnumMap?keys as key><#if remitFlow.status == key><span>${remitStatusEnumMap[key]!'' }</span></#if></#list></#if></td>
		              </tr>
                       <i class="loading" id="loading1">
	                    	<img alt="" src="${RESOURCE_IMG}/images/loading.gif"/>
	                   </i>
                      <#if remitFlow.remitChannel != 0>
                      	  <tr bgcolor="#e4f4fa">
			               ...
			               <tr bgcolor="#e4f4fa">
                    		<th height="22">买方付款回单：</th>
                    			<td id="file2_td">
                      				<span class="w-140">买方付款回单：</span><input name="file2" id="file2" onchange="uploadFile(this);"  value=""  type="file" style="width:180px;" />
			                    </td>
                    		<th>卖方付款回单：</th>
			                    <td id="file1_td">
			                    	<span class="w-140">卖方付款回单：</span><input name="file1" id="file1" onchange="uploadFile(this);"  value=""  type="file" style="width:180px;" />
			                    </td>
                		  </tr>
                		   <tr bgcolor="#ffffff">
			                    <th height="22">分润完成时间：</th>
			                    <td colspan="3">
...
function uploadFile(obj){
	...
	if(!/.(gif|jpeg|bmp|jpg|png)$/.test(file)){
    	...
    	$("#loading1").show();
    	var fileId = obj.id;
    	var url = "/getRemitManage/uploadFile";
    	 $.ajaxFileUpload({
    	   		url:url,
    	   		type: 'POST',
    	   		data : {fileId:fileId},
...
function checkRefundInfo(type){
	var url = "/getRemitManage/doRemit?type="+type;
	var memo = $("#memo").val();
