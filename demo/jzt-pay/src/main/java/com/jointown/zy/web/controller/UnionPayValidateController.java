 ...
 * date 2015-2-5
 */
@Controller(value="unionPayValidateController")
@RequestMapping(value="/b2b")
public class UnionPayValidateController extends UserBaseController {
	 ...
	 * @return
	 */
	@RequestMapping(value="/refundUrl")
	public String refundUrl(HttpServletRequest request){
		log.info("refund接收后台通知开始");
		try {
			// 获取请求参数中所有的信息
			Map<String, String> reqParam = getAllRequestParam(request);
			// 打印请求报文
			LogUtil.printRequestLog(reqParam);
			boolean validateFlag = payValidateService.payValidate(reqParam);
			if (validateFlag) {
			...
			log.info("refund接收后台通知结束");
		} catch (Exception e) {
			log.error("refund notify validate error is:", e);
		}
		return null;
	}
