...
<div class="area-1200 clearfix">
    ...
    <div class="hy-right fr">
        <div class="pickcard-box">
            ...
            <p class="fontstyle"><strong>当前订单状态：</strong>平台已备货
            ...
            <#if order.appealState==0||order.examineState==3>
			<p class="fontstyle" id="pay_line1">1. 点击这里 <a href="/order/payOrder?orderId=${order.orderid!""}&amtType=1&userId=${order.buyerId!""}" class="btn-gray">立即付款</a> 
				<!-- 正常订单（非账期）订单  显示还有多少天支付货款 -->
	            <#if order.orderType==1>
				您还有 <strong class="col_red f14" id="day">0</strong>  天 <strong class="col_red f14" id="hms">00:00:00</strong> 支付货款 
				</#if>
			</p>
            </#if>
            <#if order.appealState==0>
            	<p class="fontstyle" id="pay_line2">2. 如经过您对药材质量有异议，可以  &nbsp;&nbsp;<a href="/order/applyReimburse?orderId=${order.orderid!""}" class="col_blue">提出申诉</a> </p>
            </#if>
...
<script type="text/javascript" >
	...
	var h=60*60*1000;
	var d=24*60*60*1000;
	var expiretime = "${order.expiretime?datetime}";
	expiretime = expiretime.replace(/-/g,"/");
	var expire = new Date(expiretime);
	var deadline = expire.getTime();
