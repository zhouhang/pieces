        ...
        <form id="legalizePerson" action="/submitPersonCertify" method="post">
            <ul class="form-2 mt15">
                <li>
                    ...
                    <input type="text" class="text-2 text-Valid" name="name" value="${ucPersonCertify.name!'' }" ajaxurl="" nullmsg="请输入您的真实姓名。" datatype="zh2-10" errormsg="请正确输入您正确的真实姓名（仅支持2-10个中文）。">
                ...
                <li>
                    ...
                    <input type="text" class="text-2 text-Valid" name="idCard" value="${ucPersonCertify.idCard!'' }"  datatype="sfz" nullmsg="请输入您的身份证号码（由15位、18位数字或字母组成）。" errormsg="请输入15位、18位数字或字母组成的身份证号码。">
                </li>
                <li>
                    <label><i class="red">*</i>上传身份证：</label>
                    <span class="col_999">上传身份证的文字提示：照片支持jpg、gif、bmp、png格式，图片最大不要超过4MB <a href="http://help.54315.com/view-1209124341-1715159533.html"  class="col_red2" target="_blank">图片太大如何处理？</a></span>
                </li>
                <li class="coption">
                ...
                <li class="mid tj-btn"><input type="submit" id="subbtn" class="yellow-btn3" value="重新提交" /></li>
...
<script type="text/javascript">
	...
	$("#legalizePerson").Validform({
	    ...
	    /* datatype:{//传入自定义datatype类型，可以是正则，也可以是函数（函数内会传入一个参数）;
			"ppath":function(gets,obj,curform,regxp){
				var picPath = $("#picBigPath").val();
 				...
 				if(picPath == ""){
 					bghui();
 					Alert({str:'请重新上传身份证正面照!'});
 					return false;
 				}
 				if(picPath1 == ""){
 					bghui();
 					Alert({str:'请重新上传身份证反面照!'});
 					return false;
 				}
 				return true;
			}
	    ...
	    callback:function(data){
	    	if(data.status=='yes'){
	    		...
	    		$("#subbtn").val("重新提交成功");
	    		Alert({
	    			str:'重新提交资料成功！我们会在1-3个工作日内为您审核，请耐心等待！',
	    			buttons:[{
	    				name:'确定',
	    				id:'1',
	    				classname:'btn-style',
	...
	//提交个人实名认证审核资料
	 /* $("#subbtn").click(function(){
		...
		var _certifyId = $("input[name=certifyId]").val();
