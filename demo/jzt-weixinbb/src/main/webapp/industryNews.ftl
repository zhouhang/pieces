...
<body>
	...
	<div id="conts">
		<div class="bge5" id="bge5_11" style="display: block;">
			...
			</#list> <input type="button" class="btn-1 more" id="more_11"
				value="查看更多">
		</div>
	</div>

	<input type="hidden" id="pageNo_11" name="pageNo_11" value="${pageNo_1}"/>
		...
		$(function() {
			...
			$('#search').click(
					function() {
						...
						$('#pz_search').val(pz);
						$.get('/industryNews_ajax?lmid=11&pageNo=0&pz=' + pz,
								function(data) {
									$("#bge5_11 .box-report").remove();
									$("#pageNo_11").val(1);
									addArticles(data, 11);
								}, 'json');
					});
